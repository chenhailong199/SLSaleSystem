...
<div>
	<ul class="breadcrumb">
		...
		<li><a href="/SL/informanage/portalinfoList.html">资讯列表</a></li>
	</ul>
</div>
			<div class="row-fluid sortable">		
				<div class="box span12">
					...
					<!-- 
					<form action="/informanage/portalinfoList.html" method="post">
							<div class="control-group">
								<label class="control-label" for="appendedInputButton">按资讯标题搜索：</label>
								<div class="controls">
						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <c:forEach items="${page.items}" var="infor">
							<tr>
								...
								<td class="center"><a href="/SL/informanage/portalInfoDetail.html?id=${infor.id}">${infor.title}</a></td>
					...
					<div class="pagination pagination-centered">
					  <ul>
					   <c:choose>
					  	...
					  	<c:otherwise>
					  	<li><a href="/SL/informanage/portalinfoList.html?p=1&k=${k}" title="首页">首页</a></li>
					  	</c:otherwise>
					   </c:choose>
						<c:if test="${page.prevPages!=null}">
							<c:forEach items="${page.prevPages}" var="num">
								<li><a href="/SL/informanage/portalinfoList.html?p=${num}&k=${k}"
						...
						<c:if test="${page.nextPages!=null}">
							<c:forEach items="${page.nextPages}" var="num">
								<li><a href="/SL/informanage/portalinfoList.html?p=${num}&k=${k}" title="${num}">
								${num} </a></li>
							</c:forEach>
						</c:if>
						<c:if test="${page.pageCount !=null}">
							<c:choose>
						  	...
						  	<c:otherwise>
						  	<li><a href="/SL/informanage/portalinfoList.html?p=${page.pageCount}&k=${k}" title="尾页">尾页</a></li>
						  	</c:otherwise>
						    </c:choose>
					    </c:if>
						<c:if test="${page.pageCount == null}">
