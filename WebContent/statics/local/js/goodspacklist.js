$('.addGoodsPack').click(function(e){
	window.location.href="/SL/backend/addgoodspack.html";
});

$('.modifygoodspack').click(function(e){
	var id = $(this).attr('id');
	window.location.href="/SL/backend/modifygoodspack.html?id="+id;
});

$('.viewgoodspack').click(function(e){
	var id = $(this).attr('id');
	window.location.href="/SL/backend/viewgoodspack.html?id="+id;
});
...
$('.delgoodspack').click(function(e){
	...
	if(confirm("您确定要删除【"+goodsPackName+"】这个商品套餐吗？")){
		//delete
		$.post("/SL/backend/delgoodspack.html",{'delId':d_id},function(result){
			if("success" == result){
				alert("删除成功！");
				window.location.href="/SL/backend/goodspacklist.html";
			}else{
				alert("删除失败！");
			}
		},'html');
...
$(".modifystate").click(function(){
	...
	$.ajax({
		url: '/SL/backend/modifygoodspackstate.html',
		type: 'POST',
