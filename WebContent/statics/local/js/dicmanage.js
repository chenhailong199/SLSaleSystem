...
$("#modifytypeCode").blur(function(){
	...
	if(mlc != ""){
		$.post("/SL/backend/typecodeisexit.html",{'typeCode':mlc,'id':$("#modifydicid").val()},function(result){
			if(result == "repeat"){
...
$(".typecodelist").click(function(){
	...
	//getJsonDic tcode
	$.ajax({
		url: '/SL/backend/getJsonDic.html',
		type: 'POST',
		data:{"tcode":typecode},
		dataType: 'JSON',
		timeout: 1000,
		...
		success: function(result){
			if("failed" == result){
				...
				for(var i=0;i<result.length;i++){
					...
					str += "<div>类型代码:"+result[i].typeCode+"</div>";
					str += "<div>类型名称:"+result[i].typeName+"</div>";
					
					str += "<div>数据数值:<input type=\"text\" disabled=\"disabled\" onkeyup=\"this.value=this.value.replace(\/\\D\/g,'')\" onafterpaste=\"this.value=this.value.replace(\/\\D\/g,'')\" id=\"valueIdText"+result[i].id+"\" value=\""+result[i].valueId+"\"/></div>";
					str += "<div>数值名称:<input type=\"text\" id=\"valueNameText"+result[i].id+"\" value=\""+result[i].valueName+"\"/></div>";
					str += "<div class=\"editdiv\">";
					str += "<img class=\"modifyDicValue\" modifytypecode="+result[i].typeCode+" modifytypename="+result[i].typeName+" id=\""+result[i].id+"\" valueid=\""+result[i].valueId+"\" valuename=\""+result[i].valueName+"\" src=\"/SL/statics/bootstrap/img/ico10.png\"> <img class=\"delDicValue\" deltypename="+result[i].typeName+" id=\""+result[i].id+"\" valueid=\""+result[i].valueId+"\" valuename=\""+result[i].valueName+"\"  src=\"/SL/statics/bootstrap/img/linkdel.png\">";
					str += "</div>";
					str += "<div id=\"dicTip"+result[i].id+"\"></div>";
					str += "</li>";
					maxValue = result[i].valueId;
				}
...
$("#modifyDicExeBtn").click(function(){
	...
	if(dic.typeCode == ""){
		...
		$.ajax({
			...
			success: function(result){
				if(result != "" && "success" == result){
					window.location.href = "/SL/backend/dicmanage.html";
				}else if("failed" == result){
					tip.css("color","red");
					tip.html("数据字典修改失败！请重试。");
				}else if("nodata" == result){
...
$(".delMainDic").click(function(){
	...
	if(confirm("删除操作会删除\""+dictypename+"\"下所有的字典值，您确定要删除\""+dictypename+"\"吗？")){
		...
		$.ajax({
			url: '/SL/backend/delMainDic.html',
			type: 'POST',
