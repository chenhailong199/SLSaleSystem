...
function loadingRoleList(){
	window.location.href = "/SL/backend/rolelist.html";
}
//保存新增角色
...
$(".modifyrole").click(function(){
	...
	if(roleCode == "" || roleCode == null){
		...
		var tip = "您确定要将\n角色代码：["+oldCode + "] 修改为 ["+roleCode + "]\n角色名称：[" + oldName + "] 修改为 [" + roleName + "] 吗？";
		if(confirm(tip)){
			var role = new Object();
			role.id = id;
			...
			$.ajax({
				url: '/SL/backend/modifyRole.html',
				type: 'POST',
...
$(".modifyIsStart").click(function(){
	...
	$.ajax({
		url: '/SL/backend/modifyRoleStatus.html',
		type: 'POST',
